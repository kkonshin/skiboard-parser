...
$itemsList = $items->getList(["NAME" => $doublesFilter[0], "SECTION_ID" => ''])->list;
//$itemsList = $items->getList(["NAME" => $doublesFilter[0], "SECTION_ID" => ''], ["PROPERTY_CATEGORY_ID"])->list;

foreach ($itemsList as $item) {
	$itemsIds[$item["ID"]] = $item["ID"];
}

// FIXME нужно получить все элементы с таким именем, прежде чем выбирать привязку по ID
